...
def compare_simple_method():
    ...
    for sentiment in classified_tweets:
        print(sentiment)
        print(len(classified_tweets[sentiment]))
...
def compare_simple_method_balanced():
    print("Starting method validation.")
    print("Classifying tweets according to word occurences.")
    ...
    # Balance the tweets by removing the tweets with the least number of words from sentiments that have more than the minimum
    for sentiment in classified_tweets:
        if len(classified_tweets[sentiment]) > min_tweet_count:
            # Sort tweets by the number of words (ascending)
            classified_tweets[sentiment].sort(key=lambda tweet: len(eval(tweet)))
            # Keep only the top min_tweet_count tweets
            classified_tweets[sentiment] = classified_tweets[sentiment][:min_tweet_count]

    # Validate that each sentiment has the same number of tweets
    for sentiment in classified_tweets:
        print(f"{sentiment} tweets: {len(classified_tweets[sentiment])}")
...
if __name__ == "__main__":
    compare_simple_method_balanced()